- [mini-react](#mini-react)
  ...
  - [scheduleUpdateOnFiber 調度更新開始](#scheduleupdateonfiber-調度更新開始)
    ...
    - [react-reconciler workLoop](#react-reconciler-workloop)
      ...
      - [第一階段 workLoopSync -\> performUnitOfWork()：兩階段 beginWork, completeUnitOfWork](#第一階段-workloopsync---performunitofwork兩階段-beginwork-completeunitofwork)
        ...
        - [completeUnitOfWork - 按照 fiber tag 建立真實 DOM](#completeunitofwork---按照-fiber-tag-建立真實-dom)
          - [看 fiber 建立的結果](#看-fiber-建立的結果)
      - [第二階段 commit: VDOM -\> DOM](#第二階段-commit-vdom---dom)
        - [可以看到 `completeWork` 和 `commitMutationEffects` 中，都有用到 `appendChild`，他們有什麼不同？為什麼 `completeWork` 要去操作虛擬 DOM?](#可以看到-completework-和-commitmutationeffects-中都有用到-appendchild他們有什麼不同為什麼-completework-要去操作虛擬-dom)
  - [補充 協調 DIFF 的流程圖](#補充-協調-diff-的流程圖)
  - [補充各種節點渲染](#補充各種節點渲染)
    - [文本節點](#文本節點)
      - [第一種: .render("xxx")](#第一種-renderxxx)
      - [第二種: children fibers Array 中有文字節點](#第二種-children-fibers-array-中有文字節點)
    - [Fragment](#fragment)
      - [巢狀的 Framgment/根節點就是 Fragment](#巢狀的-framgment根節點就是-fragment)
  ...
  - [Hooks](#hooks)
    - [模擬 useReducer](#模擬-usereducer)
      - [定義 useReducer](#定義-usereducer)
        ...
        - [dispatch 事件，修改 hook.memoizedState](#dispatch-事件修改-hookmemoizedstate)
        - [render 階段](#render-階段)
    - [節點刪除](#節點刪除)
    - [多節點 DIFF](#多節點-diff)
  ...
  - [Context](#context)
    - [模擬 context](#模擬-context)
      ...
      - [開始寫 context](#開始寫-context)
        - [補充源碼](#補充源碼)
          - [為什麼 Provider props.value 發生改變後，需要主動去找消費者並發起調度？不能子消費者，在處理時，判斷 context 有沒有變化就好了嗎？](#為什麼-provider-propsvalue-發生改變後需要主動去找消費者並發起調度不能子消費者在處理時判斷-context-有沒有變化就好了嗎)
          - [消費者不必在 Provider 底下，也可以拿到 context 的預設值，只是為了解決跨層級傳遞的話，那這樣跟直接導入變數使用有什麼區別？](#消費者不必在-provider-底下也可以拿到-context-的預設值只是為了解決跨層級傳遞的話那這樣跟直接導入變數使用有什麼區別)
    - [Consumer](#consumer)
...
- 指令
  - `pnpm run dev` - 用來開發 mini-react，啟動 examples 項目
...
- [鏈表整理筆記](./linked-list.md)
- [react v18 重點整理](./react_v18.md)
...
```rust
用戶觸發原生事件（如 click、change、keydown 等）
   |
瀏覽器從根節點開始捕獲事件，直到目標元素（捕獲階段）
   |
React 的頂層事件監聽器（root container 上的事件監聽），攔截所有原生事件
   |
將瀏覽器事件包裝成 React 合成事件對象（SyntheticEvent 跨瀏覽器，提供統一的 API）
   |
檢查事件對應的捕獲處理器（onClickCapture 等）
   |
   --> 存在捕獲處理器 --> 執行處理器（從根到目標元素）
   |
   --> 不存在捕獲處理器 --> 跳過捕獲階段
   |
檢查事件對應的冒泡處理器（onClick 等）
   |
   --> 存在冒泡處理器 --> 執行處理器（從目標元素向上冒泡到根）
   |
   --> 不存在冒泡處理器 --> 結束處理
   |
合成事件對象釋放（設置屬性為 null，供 GC 回收）
   |
React 判斷是否需要更新 DOM 或執行 setState
  |
合成事件執行完成，進入更新調度階段
   |
根據事件類型和當前上下文確定優先級
   |
   --> 高優先級（如用戶輸入） --> 立即執行對應的更新任務
   |
   --> 低優先級（如閒置數據更新） --> 延遲執行，等待空閒時間處理
   |
React 調度器（Scheduler）管理任務隊列，分配執行時間
   |
開始協調（Reconciliation）和渲染（Rendering）
   |
更新完成，進入 commit 階段
   |
DOM 更新並反映至頁面
